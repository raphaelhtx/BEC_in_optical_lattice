...
#!/usr/bin/env python3
# -*- coding: utf-8 -*-
# File: 2D_fingerprinting_4systems.py
# Author: Raphaël Heurtaux
# Description: Algorithme GRAPE appliqué à 4 systèmes BEC pour implémenter la méthode de fingerprinting dans un réseau bidimensionnel. Code optimisé pour GPU (nécessite CUDA)
...
delta = 1e-3
theta_max = np.pi
sys1 = BEC_2D(mmax, nmax, q, s, lamb=delta, theta=np.pi/6)
sys2 = BEC_2D(mmax, nmax, q, s, lamb=delta, theta=5*np.pi/6)
sys3 = BEC_2D(mmax, nmax, q, s, lamb=-delta, theta=np.pi/6)
sys4 = BEC_2D(mmax, nmax, q, s, lamb=-delta, theta=5*np.pi/6)
...
psit1_gpu = cp.zeros(d, dtype=cp.complex128); psit1_gpu[sys1.get_k( 1, 0)] = 1
psit2_gpu = cp.zeros(d, dtype=cp.complex128); psit2_gpu[sys2.get_k( 0, 1)] = 1
psit3_gpu = cp.zeros(d, dtype=cp.complex128); psit3_gpu[sys3.get_k(-1, 0)] = 1
psit4_gpu = cp.zeros(d, dtype=cp.complex128); psit4_gpu[sys4.get_k( 0,-1)] = 1
...
problem.add_system(sys4, psi0_gpu, psit4_gpu)
...
path = f"./data/optimal_controls/{nom_fichier}_{thold}us_s_{s}.npz"
if new_optimisation:
    # Optimisation L-BFGS-B
...
Psi4_gpu, _, _ = prop.forward(sys4, psi0_gpu, uopt_gpu)
...
pop4 = float(abs(cp.vdot(psit4_gpu, Psi4_gpu[:, -1])) ** 2)

print(f"Population sys1 dans |1,0⟩ : {pop1:.8f}")
print(f"Population sys2 dans |0,1⟩ : {pop2:.8f}")
print(f"Population sys3 dans |-1,0⟩ : {pop3:.8f}")
print(f"Population sys4 dans |0,-1⟩ : {pop4:.8f}")
print(f"Fidélité moyenne              : {(pop1+pop2+pop3+pop4)/4:.8f}")
...
#----------------------------------------------------------------------------------
#
# Calcul des résultats des simulations
#
#----------------------------------------------------------------------------------
...
N_theta = 50
N_lambda = 40
theta_tab = np.linspace(0, 2*np.pi, N_theta)
lambda_tab = np.linspace(-delta, delta, N_lambda)

N_lambda_geo = 5
N_theta_geo = 11
lambda_tab_geo = np.linspace(-delta, delta, 5)
theta_tab_geo = np.array([0, np.pi/6, np.pi/4, np.pi/3, np.pi/2, 5*np.pi/6, np.pi, 7*np.pi/6, 5*np.pi/4, 3*np.pi/2, 11*np.pi/6])
...
if new_plots:
    ...
    P4_theta = np.zeros((N_lambda, N_theta_geo))
    ...
    for i, lamb in enumerate(lambda_tab):
        for j, theta in enumerate(theta_tab_geo):
            ...
            popl4 = abs(cp.vdot(psit4_gpu, Psil_gpu[:,-1]))**2
    ...
    P4_lambda = np.zeros((N_lambda_geo, N_theta))
    ...
    np.savez(path, Theta=Theta, Lambda=Lambda, P1=P1, P2=P2, P3=P3, P4=P4, P1_theta=P1_theta, P2_theta=P2_theta, P3_theta=P3_theta, P4_theta=P4_theta, P1_lambda=P1_lambda, P2_lambda=P2_lambda, P3_lambda=P3_lambda, P4_lambda=P4_lambda)
...
p4_tab = np.zeros(N_theta)
p1234_tab = np.zeros(N_theta)
x1_tab = np.zeros(N_theta)
x2_tab = np.zeros(N_theta)
x3_tab = np.zeros(N_theta)
x4_tab = np.zeros(N_theta)
x1234_tab = np.zeros(N_theta)

for i, lamb in enumerate(lambda_tab_geo):
    for j, theta in enumerate(theta_tab):

        alpha = np.pi/4 - np.pi/2*lamb/delta*np.sin(theta)
        beta = np.pi/4 - np.pi/(2*np.sqrt(3))*lamb/delta*np.cos(theta)

        x1 = np.cos(alpha) * np.cos(beta)
        x2 = np.cos(alpha) * np.sin(beta)
        x3 = np.sin(alpha) * np.sin(beta)
        x4 = np.sin(alpha) * np.cos(beta)
        ...
        print(f"Population dans les état 1,2,3 : {popl1 + popl2 + popl3 + popl4}")

        p1_tab[j] = popl1
        p2_tab[j] = popl2
        p3_tab[j] = popl3
        p4_tab[j] = popl4
        p1234_tab[j] = popl1 + popl2 + popl3 +popl4

        x1_tab[j] = x1*x1
        x2_tab[j] = x2*x2
        x3_tab[j] = x3*x3
        x4_tab[j] = x4*x4
        x1234_tab[j] = x1*x1 + x2*x2 + x3*x3 +x4*x4
    ...
    plt.plot(theta_tab, p4_tab, marker='o', linestyle='-', color='C4')
    plt.plot(theta_tab, p1234_tab, marker='o', linestyle='-', color='C5')
    ...
    plt.plot(theta_tab, x4_tab, marker='o', linestyle=':', color='C4')
    plt.plot(theta_tab, x1234_tab, marker='o', linestyle=':', color='C5')

    plt.xlabel(r'$\theta$', fontsize=14)
    plt.ylabel(r'$p\pm1$', fontsize=14)
    plt.title(fr"Population des états $|-\!1\rangle$ et $|1\rangle$  à $t^*$ selon la force $\theta$, pour $\lambda=${lamb}")
...
p4_tab = np.zeros(N_lambda)
p1234_tab = np.zeros(N_lambda)
x1_tab = np.zeros(N_lambda)
x2_tab = np.zeros(N_lambda)
x3_tab = np.zeros(N_lambda)
x4_tab = np.zeros(N_lambda)
x1234_tab = np.zeros(N_lambda)
...
for j, theta in enumerate(theta_tab_geo):
    for i, lamb in enumerate(lambda_tab):
        ...
        print(f"Population dans les état 1,2,3,4 : {popl1 + popl2 + popl3 +popl4}")
        print(f"Population dans l'état 1 = {popl1:.3f} VS x1² conjecturé = {x1*x1:.3f}")

        p1_tab[i] = popl1
        p2_tab[i] = popl2
        p3_tab[i] = popl3
        p4_tab[i] = popl4
        p1234_tab[i] = popl1 + popl2 + popl3 +popl4

        x1_tab[i] = x1*x1
        x2_tab[i] = x2*x2
        x3_tab[i] = x3*x3
        x4_tab[i] = x4*x4
        x1234_tab[i] = x1*x1 + x2*x2 + x3*x3 +x4*x4
    ...
    plt.plot(lambda_tab, p4_tab, marker='o', linestyle='-', color='C4')
    plt.plot(lambda_tab, p1234_tab, marker='o', linestyle='-', color='C5')
    ...
    plt.plot(lambda_tab, x4_tab, marker='o', linestyle=':', color='C4')
    plt.plot(lambda_tab, x1234_tab, marker='o', linestyle=':', color='C5')

    plt.xlabel(r'$\lambda$', fontsize=14)
    plt.ylabel(r'$p\pm1$', fontsize=14)
    plt.title(fr"Population des états $|-\!1\rangle$ et $|1\rangle$  à $t^*$ selon la force $\lambda$, pour $\theta=${theta}")
    plt.grid(True, linestyle='--', alpha=0.7)
    plt.ylim(0,1)
    plt.legend()
    plt.show()
...
plt.xlabel(r"$\theta$")
plt.ylabel(r"$\lambda$")
plt.title(r"Population simulée $p_1(\lambda,\theta)$")
plt.tight_layout()
plt.show()


plt.figure(figsize=(5,4))
plt.contourf(Theta, Lambda, P2, levels=100, cmap="magma")
plt.colorbar(label=r"$p_2$")
plt.xlabel(r"$\theta$")
plt.ylabel(r"$\lambda$")
plt.title(r"Population simulée $p_2(\lambda,\theta)$")
plt.tight_layout()
plt.show()
...
plt.figure(figsize=(5,4))
plt.contourf(Theta, Lambda, P3, levels=100, cmap="magma")
plt.colorbar(label=r"$p_3$")
plt.xlabel(r"$\theta$")
plt.ylabel(r"$\lambda$")
plt.title(r"Population simulée $p_3(\lambda,\theta)$")
plt.tight_layout()
plt.show()

plt.figure(figsize=(5,4))
plt.contourf(Theta, Lambda, P4, levels=100, cmap="magma")
plt.colorbar(label=r"$p_4$")
plt.xlabel(r"$\theta$")
plt.ylabel(r"$\lambda$")
plt.title(r"Population simulée $p_4(\lambda,\theta)$")
plt.tight_layout()
plt.show()


plt.figure(figsize=(5,4))
plt.contourf(Theta, Lambda, P1+P2+P3+P4, levels=100, cmap="magma")
plt.colorbar(label=r"$p_1^2+p_3^2+p_3^2+p_4^2$")
plt.xlabel(r"$\theta$")
plt.ylabel(r"$\lambda$")
plt.title(r"$p_1^2+p_3^2+p_3^2+p_4^2$")
plt.tight_layout()
plt.show()
...
# 4
error_4 = np.abs(P4-X4)
plt.figure(figsize=(6,4))
plt.contourf(Theta, Lambda, error_4, 
             levels=100, cmap="magma")
plt.colorbar()
plt.xlabel(r"$\theta$")
plt.ylabel(r"$\lambda$")
plt.title("Erreur pop 4 : modèle vs simulation")
plt.tight_layout()
plt.show()
...
fidelity_sys4 = np.array([abs(cp.vdot(psit4_gpu, Psi4_gpu[:, i]).get())**2 for i in range(Nt)])

plt.figure(figsize=(8, 5))
plt.plot(time_us, fidelity_sys1, label="Fidélité sys1 (vers |1,0⟩)")
...
plt.plot(time_us, fidelity_sys4, linestyle=':', label="Fidélité sys4 (vers |0,-1⟩)")
